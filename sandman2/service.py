...
def jsonify_list(resource_l):
    ...
    response = flask.jsonify([resource.to_dict() for resource in resource_l])
    if len(resource_l) > 0:
        response = add_link_headers(response, resource_l[0].links())
    return response
...
class Service(MethodView):
    ...
    def delete(self, resource_id):
        ...
        """
        raise BadRequestException('Forbidden to retrieve/export all, delete')
        '''
